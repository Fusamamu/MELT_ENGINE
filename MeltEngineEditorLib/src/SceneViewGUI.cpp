...
namespace MELT_EDITOR
{
    ...
    ImVec2 remap_im_vec2(const ImVec2& value,
    ...
    void SceneViewGUI::draw_gui()
    {
        ...
        if (ImGui::Begin("Scene view"))
        {
            ...
            if (ImGui::IsWindowHovered())
            {
                if (ImGui::IsMouseClicked(ImGuiMouseButton_Left))
                {
                    if (!ImGuizmo::IsOver() && !ImGuizmo::IsUsing() || !_working_scene->selected_node_id.has_value())
                    {
                        ...
                        m_engine->select_object(MELT::Input.MouseScreenPosition, m_engine->main_camera);
                    }
                }
            }
            ...
            ImVec2 _normalizedPos = remap_im_vec2(_screenPos,
                                                0.0f, _scene_editor_window_width , 0.0f, 1.0f,
                                                0.0f, _scene_editor_window_height, 0.0f, 1.0f);

            ImVec2 _mouseWorldPos = remap_im_vec2(_screenPos,
                                                0.0f, _scene_editor_window_width , -m_engine->main_camera.HalfScreenWidth(),  m_engine->main_camera.HalfScreenWidth(),
                                                0.0f, _scene_editor_window_height,  m_engine->main_camera.HalfScreenHeight(), -m_engine->main_camera.HalfScreenHeight());

            _mouseWorldPos.x += m_engine->main_camera.position.x;
            _mouseWorldPos.y += m_engine->main_camera.position.y;
            ...
            ImGui::Text("Orthographic projection W H : (%.1f, %.1f)"     , m_engine->main_camera.ScreenSize.x            , m_engine->main_camera.ScreenSize.y            );
            ...
            ImGui::InputFloat3("Camera position", glm::value_ptr(m_engine->main_camera.position));
            ImGui::SliderFloat("Near plane"       , &m_engine->main_camera.near_plane, -100.0f, 0.0f);
            ImGui::SliderFloat("Far plane"        , &m_engine->main_camera.far_plane ,     0.0, 1000.0f);
            ImGui::SliderFloat("Orthographic size", &m_engine->main_camera.orthographic_size, 1.0f, 200.f);

            auto _ent_view = _working_scene->ecs_registry.view<MELT::Transform, MELT::Gizmos>();

            for (auto _entity : _ent_view)
            {
                auto& _transform = _ent_view.get<MELT::Transform>(_entity);
                auto& _gizmos    = _ent_view.get<MELT::Gizmos>   (_entity);
                    ...
                    screenPos.y >= winMin.y && screenPos.y <= winMax.y)
                {
                    ...
                    MELT::TextureData* _icon_texture_data = nullptr;

                    switch (_gizmos.type)
                    {
                        case MELT::Gizmos::Type::LIGHT:
                            _icon_texture_data = m_engine->manager_registry.get<MELT::ResourceManager>()->get_texture_data("light_icon.png");
                            break;
                        case MELT::Gizmos::Type::CAMERA:
                            _icon_texture_data = m_engine->manager_registry.get<MELT::ResourceManager>()->get_texture_data("camera_icon.png");
                            break;
                        default: ;
                    }

                    ImTextureID _icon_id = (ImTextureID)(intptr_t)_icon_texture_data->p_texture->texture_id;
                    ...
                    ImGui::GetWindowDrawList()->AddImage(_icon_id, drawMin, drawMax);
                }
            }

...
namespace MELT
{
    ...
    SceneNode& Scene::create_node(const std::string& _name)
    {
    ...
    void Scene::destroy_node(const SceneNode& _node)
    {
        entt::entity entity = _node.get_entity();
        if (!ecs_registry.valid(entity))
            return;
        ecs_registry.destroy(entity);
        auto it = std::remove_if(m_nodes.begin(), m_nodes.end(), [&](const SceneNode& node) {
    ...
    {
        auto _it = std::find_if(m_nodes.begin(), m_nodes.end(), [&](const SceneNode& _node)
        {
            return _node.id == _id;
        });
